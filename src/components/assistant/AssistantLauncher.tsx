...
export function AssistantLauncher({ variant, onBeforeOpen }: Props) {
  ...
  return (
    <button
      ...
      data-assistant-launcher={variant}
      hidden={isOpen}
      onClick={(event) => {
        onBeforeOpen?.();
      ...
      className={
        variant === "desktop"
          ...
          : "assistant-launcher assistant-launcher-mobile z-40 size-14 shrink-0 items-center justify-center rounded-full border border-amber-200/70 bg-black p-0.5 shadow-[0_10px_28px_rgba(0,0,0,0.42)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-300 focus-visible:ring-offset-2 focus-visible:ring-offset-[#0b1324]"
      }
    >
