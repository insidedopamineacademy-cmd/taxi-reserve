            ...
            <li key={payment.id} className="rounded-xl border border-app-border bg-surface p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted">{payment.methodLabel}</p>
                  <p className="mt-1 text-xs text-subtle">{payment.dateLabel}</p>
                </div>
                <p className="shrink-0 text-lg font-semibold text-white tnum">{payment.amount}</p>
              </div>
              {payment.notes ? (
